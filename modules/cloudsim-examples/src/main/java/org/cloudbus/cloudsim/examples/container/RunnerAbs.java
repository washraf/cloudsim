...
import org.cloudbus.cloudsim.container.containerSelectionPolicies.PowerContainerSelectionPolicyMaxChangeRate;
import org.cloudbus.cloudsim.container.containerSelectionPolicies.PowerContainerSelectionPolicyMaximumUsage;
import org.cloudbus.cloudsim.container.containerSelectionPolicies.PowerContainerSelectionPolicyMaximumVariance;
import org.cloudbus.cloudsim.container.containerSelectionPolicies.PowerContainerSelectionPolicyMedianUsage;
import org.cloudbus.cloudsim.container.containerSelectionPolicies.PowerContainerSelectionPolicyMinChangeRate;
import org.cloudbus.cloudsim.container.containerSelectionPolicies.PowerContainerSelectionPolicyMinimumMigrationTime;
import org.cloudbus.cloudsim.container.containerSelectionPolicies.PowerContainerSelectionPolicyMinimumUsage;
import org.cloudbus.cloudsim.container.containerSelectionPolicies.PowerContainerSelectionPolicyMinimumVariance;
import org.cloudbus.cloudsim.container.core.*;
import org.cloudbus.cloudsim.container.hostSelectionPolicies.*;
import org.cloudbus.cloudsim.container.resourceAllocatorMigrationEnabled.PowerContainerVmAllocationPolicyMigrationAbstractHostSelection;
import org.cloudbus.cloudsim.container.resourceAllocatorMigrationEnabled.PowerContainerVmAllocationPolicyMigrationAnt;
...
/**
 * The RunnerAbs Class is the modified version of
 * {@link org.cloudbus.cloudsim.examples.power.RunnerAbstract} Created by 
 Walid on 1/10/17.
 */
public abstract class RunnerAbs {
	...
	public RunnerAbs(boolean enableOutput, boolean outputToFile,
			String inputFolder, String outputFolder, String vmAllocationPolicy,
			String containerAllocationPolicy, String vmSelectionPolicy,
			String containerSelectionPolicy, String hostSelectionPolicy,
			double overBookingFactor, String runTime, String logAddress) {
		setOverBookingFactor(overBookingFactor);
		setRunTime(runTime);
		setLogAddress(logAddress);
		setExperimentName(this.getExperimentName(hostSelectionPolicy,
				vmAllocationPolicy, vmSelectionPolicy,
				containerSelectionPolicy, containerAllocationPolicy,
				String.valueOf(getOverBookingFactor()), runTime));

		try {
			this.initLogOutput(enableOutput, outputToFile, outputFolder,
					vmAllocationPolicy, vmSelectionPolicy,
					containerSelectionPolicy, hostSelectionPolicy);
		...
		this.start(getExperimentName(), outputFolder, this
				.getVmAllocationPolicy(vmAllocationPolicy, vmSelectionPolicy,
						containerSelectionPolicy, hostSelectionPolicy),
				getContainerAllocationPolicy(containerAllocationPolicy));
	}
	...
	protected void initLogOutput(boolean enableOutput, boolean outputToFile,
		...
		// FileOutputStream("/home/sareh/Dropbox/programming/Results/log.txt");
		// OutputStream out = new BufferedOutputStream(new
		// FileOutputStream("/home/sareh/Dropbox/programming/Results/log1.txt"),
		// 100000);
		// Log.setOutput(out);
		if (this.isEnableOutput() && outputToFile) {
			...
			// "/stats/"+getExperimentName().substring(0, index));
			...
			File folder2 = new File(outputFolder + "/log/"
					+ getExperimentName().substring(0, index));
			...
			File folder3 = new File(outputFolder + "/ContainerMigration/"
					+ getExperimentName().substring(0, index));
			...
			File folder4 = new File(outputFolder + "/NewlyCreatedVms/"
					+ getExperimentName().substring(0, index));
			...
			File folder5 = new File(outputFolder + "/EnergyConsumption/"
					+ getExperimentName().substring(0, index));
			...
			File file = new File(outputFolder
					...
					+ getExperimentName().substring(0, index)
					+ "/"
					+ this.getExperimentName(new String[] {
							hostSelectionPolicy, vmAllocationPolicy,
							vmSelectionPolicy, containerSelectionPolicy,
							String.valueOf(getOverBookingFactor()),
							getRunTime() }) + ".txt");
			file.createNewFile();
	...
	protected void start(String experimentName, String outputFolder,
		...
		try {
			...
			// PowerContainerDatacenter e = (PowerContainerDatacenter)
			// HelperEx.createDatacenter("Datacenter",
			// PowerContainerDatacenter.class, hostList, vmAllocationPolicy,
			// containerAllocationPolicy);
			vmAllocationPolicy.setDatacenter(e);
			e.setDisableVmMigrations(false);
			broker.submitVmList(vmList);
			broker.submitContainerList(containerList);
			broker.submitCloudletList(cloudletList.subList(0,
					containerList.size()));
			;
			...
			// HelperEx.printResults(e,
			// broker.getVmsCreatedList(),broker.getContainersCreatedList()
			// ,lastClock, experimentName, true, outputFolder);
			HelperEx.printResultsNew(e, broker, lastClock, experimentName,
					true, outputFolder);
	...
	protected ContainerVmAllocationPolicy getVmAllocationPolicy(
			String vmAllocationPolicyName, String vmSelectionPolicyName,
			String containerSelectionPolicyName, String hostSelectionPolicyName) {
		...
		if (!vmSelectionPolicyName.isEmpty()
				&& !containerSelectionPolicyName.isEmpty()
				&& !hostSelectionPolicyName.isEmpty()) {
			vmSelectionPolicy = this
					.getVmSelectionPolicy(vmSelectionPolicyName);
			containerSelectionPolicy = this
					.getContainerSelectionPolicy(containerSelectionPolicyName);
			hostSelectionPolicy = this
					.getHostSelectionPolicy(hostSelectionPolicyName);
		}

		if (vmAllocationPolicyName.startsWith("MSThreshold-Over_")) {
			double overUtilizationThreshold = Double
					.parseDouble(vmAllocationPolicyName.substring(18));
			vmAllocationPolicy = new PowerContainerVmAllocationPolicyMigrationStaticThresholdMC(
					hostList, vmSelectionPolicy, containerSelectionPolicy,
					hostSelectionPolicy, overUtilizationThreshold,
					ConstantsExamples.VM_TYPES, ConstantsExamples.VM_PES,
					ConstantsExamples.VM_RAM, ConstantsExamples.VM_BW,
					ConstantsExamples.VM_SIZE, ConstantsExamples.VM_MIPS);
		} else if (vmAllocationPolicyName.startsWith("MSThreshold-Under_")) {
			double overUtilizationThreshold = Double
					.parseDouble(vmAllocationPolicyName.substring(18, 22));
			double underUtilizationThreshold = Double
					.parseDouble(vmAllocationPolicyName.substring(24));
			vmAllocationPolicy = new PowerContainerVmAllocationPolicyMigrationStaticThresholdMCUnderUtilized(
					hostList, vmSelectionPolicy, containerSelectionPolicy,
					hostSelectionPolicy, overUtilizationThreshold,
					underUtilizationThreshold, ConstantsExamples.VM_TYPES,
					ConstantsExamples.VM_PES, ConstantsExamples.VM_RAM,
					ConstantsExamples.VM_BW, ConstantsExamples.VM_SIZE,
			...
			double underUtilizationThreshold = Double
					.parseDouble(vmAllocationPolicyName.substring(24));
			vmAllocationPolicy = new PowerContainerVmAllocationPolicyMigrationAbstractHostSelection(
					hostList, vmSelectionPolicy, hostSelectionPolicy,
					overUtilizationThreshold, underUtilizationThreshold);

		} else if (vmAllocationPolicyName.startsWith("ANT")) {
			double overUtilizationThreshold = Double
					.parseDouble(vmAllocationPolicyName.substring(4, 7));
			double underUtilizationThreshold = Double
					.parseDouble(vmAllocationPolicyName.substring(9));
			vmAllocationPolicy = new PowerContainerVmAllocationPolicyMigrationAnt(
					hostList, vmSelectionPolicy, containerSelectionPolicy,
					hostSelectionPolicy, overUtilizationThreshold,
					underUtilizationThreshold, ConstantsExamples.VM_TYPES,
					ConstantsExamples.VM_PES, ConstantsExamples.VM_RAM,
	...
	protected ContainerAllocationPolicy getContainerAllocationPolicy(
			String containerAllocationPolicyName) {
		ContainerAllocationPolicy containerAllocationPolicy;
		if (containerAllocationPolicyName == "Simple") {

			containerAllocationPolicy = new PowerContainerAllocationPolicySimple(); // DVFS
																					// policy
																					// without
																					// VM
																					// migrations
			...
			containerAllocationPolicy = new ContainerAllocationPolicyRS(
					placementPolicy); // DVFS policy without VM migrations
		}
	...
	protected HostSelectionPolicy getHostSelectionPolicy(
			String hostSelectionPolicyName) {
		...
		if (hostSelectionPolicyName == "FirstFit") {
			...
			hostSelectionPolicy = new HostSelectionPolicyMinimumCorrelation(
					new HostSelectionPolicyLeastFull());
		} else if (hostSelectionPolicyName == "Cor") {
			hostSelectionPolicy = new HostSelectionPolicyCorrelation(
					new HostSelectionPolicyLeastFull());
		} 
		 else if (hostSelectionPolicyName == "MaxCor") {
				hostSelectionPolicy = new HostSelectionPolicyMaximumCorrelation(
						new HostSelectionPolicyLeastFull());
		 }else if (hostSelectionPolicyName == "RandomSelection") {
			hostSelectionPolicy = new HostSelectionPolicyRandomSelection();
		} else if (hostSelectionPolicyName == "MaxVar") {

			hostSelectionPolicy = new HostSelectionPolicyMaxVariance();
		} else if (hostSelectionPolicyName == "LeastVar") {

			hostSelectionPolicy = new HostSelectionPolicyLeastVariance();
		}
		else if (hostSelectionPolicyName == "Ant") {

			hostSelectionPolicy = new HostSelectionPolicyAnt();
		}
		else if (hostSelectionPolicyName == "MinChangeRate") {

			hostSelectionPolicy = new HostSelectionPolicyMinChangeRate();
		}
		else if (hostSelectionPolicyName == "MaxChangeRate") {

			hostSelectionPolicy = new HostSelectionPolicyMaxChangeRate();
		}
		...
		// }
			...
			System.out.println("Unknown Host selection policy: "
					+ hostSelectionPolicyName);
			System.exit(0);
		}
	...
	protected PowerContainerSelectionPolicy getContainerSelectionPolicy(
			String containerSelectionPolicyName) {
		Object containerSelectionPolicy = null;
		if (containerSelectionPolicyName.equals("Cor")) {
			containerSelectionPolicy = new PowerContainerSelectionPolicyCor(
					new PowerContainerSelectionPolicyMaximumUsage());
		...
		} else if (containerSelectionPolicyName.equals("MinUsage")) {
			containerSelectionPolicy = new PowerContainerSelectionPolicyMinimumUsage();

		} else if (containerSelectionPolicyName.equals("MedUsage")) {
			containerSelectionPolicy = new PowerContainerSelectionPolicyMedianUsage();

		} else if (containerSelectionPolicyName.equals("MinMig")) {
			containerSelectionPolicy = new PowerContainerSelectionPolicyMinimumMigrationTime();

		} else if (containerSelectionPolicyName.equals("MaxVariance")) {
			containerSelectionPolicy = new PowerContainerSelectionPolicyMaximumVariance();

		} else if (containerSelectionPolicyName.equals("MinVariance")) {
			containerSelectionPolicy = new PowerContainerSelectionPolicyMinimumVariance();
		}
		 else if (containerSelectionPolicyName.equals("MaxChangeRate")) {
				containerSelectionPolicy = new PowerContainerSelectionPolicyMaxChangeRate();

			} else if (containerSelectionPolicyName.equals("MinChangeRate")) {
				containerSelectionPolicy = new PowerContainerSelectionPolicyMinChangeRate();
			}
		...
		// }
			...
			System.out.println("Unknown Container selection policy: "
					+ containerSelectionPolicyName);
			System.exit(0);
		}
	...
	protected PowerContainerVmSelectionPolicy getVmSelectionPolicy(
		...
		if (vmSelectionPolicyName.equals("VmMaxC")) {
			vmSelectionPolicy = new PowerContainerVmSelectionPolicyMaximumCorrelation(
