...
public class ContainerDatacenter extends SimEntity {
	 ...
	 * @throws Exception
	 */
	public ContainerDatacenter(String name,
	 ...
	 * <b>NOTE:</b> You do not need to override {} method, if you use this
	 * method.
	 *
	 * @pre $none
	 * @post $none
	...
	public void processEvent(SimEvent ev) {
		...
		case containerCloudSimTags.CONTAINER_MIGRATE:
			...
			break;
		case containerCloudSimTags.HOST_MONITOR:
			processHostMonitor();
			break;
	...
	private void processHostMonitor() {

		for (ContainerHost host : getHostList()) {
			if (host.getHostState() == containerCloudSimTags.OverUtilizedHost) {

			} else if (host.getHostState() == containerCloudSimTags.UnderUtilizedHost) {
				PowerContainerVmAllocationPolicyMigrationAbstractContainerHostSelection s = (PowerContainerVmAllocationPolicyMigrationAbstractContainerHostSelection) this
						.getVmAllocationPolicy();
				//Container c = s.getContainerSelectionPolicy()
				//		.getContainerToMigrate((PowerContainerHost) host);
		...
		schedule(getId(), getSchedulingInterval(),
				containerCloudSimTags.HOST_MONITOR);
	...
	public void processContainerSubmit(SimEvent ev, boolean ack) {
		...
		for (Container container : containerList) {
			boolean result = getContainerAllocationPolicy()
					.allocateVmForContainer(container, getContainerVmList());
			if (ack) {
				int[] data = new int[3];
				...
				if (result) {
					ContainerVm containerVm = getContainerAllocationPolicy()
					...
					// Log.printConcatLine("Assigning the container#" +
					// container.getUid() + "to VM #" + containerVm.getUid());
					...
					Log.printLine(String.format(
							"Couldn't find a vm to host the container #%s",
							container.getUid()));

				}
				send(ev.getSource(), CloudSim.getMinTimeBetweenEvents(),
						containerCloudSimTags.CONTAINER_CREATE_ACK, data);
	...
	protected void processCloudletStatus(SimEvent ev) {
		...
		try {
			...
			status = getVmAllocationPolicy().getHost(vmId, userId)
					.getContainerVm(vmId, userId)
					.getContainer(containerId, userId)
					.getContainerCloudletScheduler()
	...
	protected void processVmCreate(SimEvent ev, boolean ack) {
		...
		if (ack) {
			...
			send(containerVm.getUserId(), CloudSim.getMinTimeBetweenEvents(),
		...
		if (result) {
			...
			containerVm.updateVmProcessing(CloudSim.clock(),
					getVmAllocationPolicy().getHost(containerVm)
	...
	protected void processVmMigrate(SimEvent ev, boolean ack) {
		...
		Log.formatLine("%.2f: Migration of VM #%d to Host #%d is completed",
				CloudSim.clock(), containerVm.getId(), host.getId());
	...
	protected void processContainerMigrate(SimEvent ev, boolean ack) {
		...
		boolean result = getContainerAllocationPolicy().allocateVmForContainer(
		...
		if (ack) {
			...
			sendNow(ev.getSource(), containerCloudSimTags.CONTAINER_CREATE_ACK,
					data);
		}

		Log.formatLine(
				"%.2f: Migration of container #%d to Vm #%d is completed",
				CloudSim.clock(), container.getId(), container.getVm().getId());
	...
	protected void processCloudlet(SimEvent ev, int type) {
		...
		// if the sender using normal send() methods
			...
			try {
				...
				Log.printConcatLine(super.getName(),
	...
	protected void processCloudletMove(int[] receivedData, int type) {
		...
		Cloudlet cl = getVmAllocationPolicy().getHost(vmId, userId)
				.getContainerVm(vmId, userId).getContainer(containerId, userId)
		...
		if (cl == null) {// cloudlet doesn't exist
			...
			if (cl.getCloudletStatusString().equals("Success")) {// if yes, send
																	// it back
																	// to user
				int[] data = new int[3];
				data[0] = getId();
			...
			if (destId == getId()) {
				ContainerVm containerVm = getVmAllocationPolicy().getHost(
						vmDestId, userId).getContainerVm(vmDestId, userId);
				if (containerVm == null) {
					failed = true;
				} else {
					...
					double fileTransferTime = predictFileTransferTime(cl
							.getRequiredFiles());
					containerVm.getContainer(containerDestId, userId)
							.getContainerCloudletScheduler()
							.cloudletSubmit(cl, fileTransferTime);
				}
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			if (cl.isFinished()) {
				...
				Log.printConcatLine(getName(), ": Warning - Cloudlet #",
			...
			double estimatedFinishTime = container
					.getContainerCloudletScheduler().cloudletSubmit(cl,
							fileTransferTime);
			...
			Log.printLine(String.format(
					"%s.processCloudletSubmit(): ClassCastException error.",
					getName()));
			c.printStackTrace();
		} catch (Exception e) {
			Log.printLine(String.format(
					"%s.processCloudletSubmit(): Exception error.", getName()));
			e.printStackTrace();
		}
	...
	protected void updateCloudletProcessing() {
		...
		if (CloudSim.clock() < 0.111
			...
			List<? extends ContainerHost> list = getVmAllocationPolicy()
			...
			for (int i = 0; i < list.size(); i++) {
				...
				double time = host.updateContainerVmsProcessing(CloudSim
						.clock());
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (ContainerVm vm : host.getVmList()) {
				for (Container container : vm.getContainerList()) {
					while (container.getContainerCloudletScheduler()
							.isFinishedCloudlets()) {
						Cloudlet cl = container.getContainerCloudletScheduler()
	 ...
	 */
	protected void setVmAllocationPolicy(
			ContainerVmAllocationPolicy vmAllocationPolicy) {
	 ...
	 *            the new vm list
	 */
	protected <T extends ContainerVm> void setContainerVmList(
			List<T> containerVmList) {
	...
	public void setContainerAllocationPolicy(
			ContainerAllocationPolicy containerAllocationPolicy) {
